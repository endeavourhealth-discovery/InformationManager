...
public class StaticConstGenerator implements Plugin<Project> {
  ...
  public void execute(String baseDir, String packagePath,String jsonIn, String javaOut) {
    if (!baseDir.endsWith("/"))
      baseDir += "/";
    ...
    try (InputStream in = new FileInputStream(jsonIn)) {
      ...
      for (VocabDef def : defs) {
        generateJava(packagePath, javaOut, def);
      }
    } catch (Exception e) {
  ...
  private void generateJava(String packagePath, String javaOut, VocabDef def) {
    generateCode(
      def,
      ...
      """
        ...
        import %s.model.tripletree.TTIriRef;
        
        import java.util.Arrays;
        import java.util.stream.Collectors;
        
        import static %s.model.tripletree.TTIriRef.iri;
        
        public enum {TYPE} implements VocabEnum {
        """.formatted(packagePath,packagePath,packagePath),
      """
            ;
