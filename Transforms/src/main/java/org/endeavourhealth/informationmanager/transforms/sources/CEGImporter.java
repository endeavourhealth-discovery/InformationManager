...
public class CEGImporter implements TTImport {
    ...
    private void fileConceptSets() throws Exception {
        TTDocument document;
                ...
                if (ext.equalsIgnoreCase("xml")) {
                    ...
                    if (qDocument.getQuery() != null) {
                        for (QueryEntity qq : qDocument.getQuery()) {
                            ...
                            Collection<String> uses = converter.getUses(qq.getIri());
                            if (uses != null && !uses.isEmpty()) {
                                TTArray usesList = new TTArray();
                                uses.forEach(ui -> usesList.add(TTIriRef.iri(ui)));
                                ttQuery.set(IM.USES, usesList);
                            }
                    ...
                    this.conceptSets.addAll(converter.getValueSets());
                }
            }
        }
    }
