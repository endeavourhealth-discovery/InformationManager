...
public class CPRDImport implements TTImport {
	...
	public void importData(TTImportConfig config) throws Exception {
		document = manager.createDocument(IM.GRAPH_CPRD_MED.getIri());
		document.addEntity(manager.createGraph(IM.GRAPH_CPRD_MED.getIri(), "CPRD medIds ",
		...
		document = manager.createDocument(IM.GRAPH_CPRD_PROD.getIri());
		document.addEntity(manager.createGraph(IM.GRAPH_CPRD_PROD.getIri(), "CPRD product ids",
	...
	private void importDrugs(String folder) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file.toFile()))) {
			...
			while (line != null && !line.isEmpty()) {
				...
				concept.setIri(IM.CODE_SCHEME_CPRD_PROD.getIri() + "Product_" + drugId);
	...
	private void importObsCodes(String folder) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file.toFile()))) {
			...
			while (line != null && !line.isEmpty()) {
				...
				concept.setIri(IM.CODE_SCHEME_CPRD_MED.getIri() + "Medical_" + medId);
