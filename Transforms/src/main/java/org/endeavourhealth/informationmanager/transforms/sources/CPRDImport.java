...
public class CPRDImport implements TTImport {
	...
	public void importData(TTImportConfig config) throws Exception {
		document = manager.createDocument(IM.GRAPH_CPRD.getIri());
		document.addEntity(manager.createGraph(IM.GRAPH_CPRD.getIri(), "CPRD medIds and product ids",
			"CPRD identifiers (including emis code ids)."));
	...
	private void importDrugs(String folder) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file.toFile()))) {
			...
			while (line != null && !line.isEmpty()) {
				...
				concept.setIri(IM.CODE_SCHEME_CPRD.getIri() + "Product_" + drugId);
				concept.setName(fields[2]);
				concept.setCode(drugId);
				concept.setScheme(IM.CODE_SCHEME_CPRD);
	...
	private void importObsCodes(String folder) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file.toFile()))) {
			...
			while (line != null && !line.isEmpty()) {
				...
				concept.setIri(IM.CODE_SCHEME_CPRD.getIri() + "Medical_" + medId);
