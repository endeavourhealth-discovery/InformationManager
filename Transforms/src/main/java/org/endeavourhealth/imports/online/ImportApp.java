...
public class ImportApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length >= 3) {
         ...
         org.endeavourhealth.imports.sources.LoadDataTester.testLoadData(cfg.getFolder(), cfg.isSecure());
    ...
    private static void importData(TTImportConfig cfg) throws Exception {
        switch (cfg.getImportType()) {
            case "all":
                ...
                break;
            case "imv1":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_IM1, cfg.getFolder());
                importer.importByType(IM.GRAPH_IM1, cfg);
                break;
            case "prsb":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_PRSB, cfg.getFolder());
                importer.importByType(IM.GRAPH_PRSB, cfg);
                break;

            case "core":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_DISCOVERY, cfg.getFolder());
                importer.importByType(IM.GRAPH_DISCOVERY, cfg);
                break;
            case "snomed":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(SNOMED.GRAPH_SNOMED, cfg.getFolder());
                importer.importByType(SNOMED.GRAPH_SNOMED, cfg);
                break;
            case "emis":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_EMIS, cfg.getFolder());
                importer.importByType(IM.GRAPH_EMIS, cfg);
                break;
            case "tpp":
            case "ctv3":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_TPP, cfg.getFolder());
                importer.importByType(IM.GRAPH_TPP, cfg);
                break;
            case "opcs4":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_OPCS4, cfg.getFolder());
                importer.importByType(IM.GRAPH_OPCS4, cfg);
                break;
            case "icd10":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_ICD10, cfg.getFolder());
                importer.importByType(IM.GRAPH_ICD10, cfg);
                break;
            case "discoverymaps":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_ENCOUNTERS, cfg.getFolder());
                importer.importByType(IM.GRAPH_ENCOUNTERS, cfg);
                break;
            case "read2":
            case "vision":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_VISION, cfg.getFolder());
                importer.importByType(IM.GRAPH_VISION, cfg);
                break;
            case "kingsapex":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_KINGS_APEX, cfg.getFolder());
                importer.importByType(IM.GRAPH_KINGS_APEX, cfg);
                break;
            case "kingswinpath":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_KINGS_WINPATH, cfg.getFolder());
                importer.importByType(IM.GRAPH_KINGS_WINPATH, cfg);
                break;

            case "ceg" :
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_CEG_QUERY, cfg.getFolder());
                importer.importByType(IM.GRAPH_CEG_QUERY, cfg);
                break;
            case "barts":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_BARTS_CERNER, cfg.getFolder());
                importer.importByType(IM.GRAPH_BARTS_CERNER, cfg);
                break;
            case "ods":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_ODS, cfg.getFolder());
                importer.importByType(IM.GRAPH_ODS, cfg);
                break;
            case "encounters":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_ENCOUNTERS, cfg.getFolder());
                ...
                break;
            case "nhstfc":
                importer = new  org.endeavourhealth.imports.sources.Importer().validateByType(IM.GRAPH_NHS_TFC, cfg.getFolder());
                importer.importByType(IM.GRAPH_NHS_TFC, cfg);
                break;
            case "tct":
                cfg.setSkipsearch(true);
